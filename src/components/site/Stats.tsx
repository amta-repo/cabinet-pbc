...
const stats = [
  ...
  { value: 25, suffix: "", label: "Experts Qualifiés" },
];
...
const Stat = ({ value, suffix, label, run }: { value: number; suffix: string; label: string; run: boolean }) => {
  ...
  return (
    <div className="group text-center">
      <div className="font-display text-5xl font-black text-gradient-gold md:text-6xl lg:text-7xl">{n}{suffix}</div>
      <div className="mt-3 h-px w-12 bg-primary/40 mx-auto transition-all duration-500 group-hover:w-20 group-hover:bg-primary" />
      <div className="mt-3 text-xs font-semibold uppercase tracking-[0.32em] text-muted-foreground">{label}</div>
    </div>
  );
};
